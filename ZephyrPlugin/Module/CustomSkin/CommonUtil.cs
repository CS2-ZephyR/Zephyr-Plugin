...
public partial class Module
{
    private async Task GetSkinFromDatabase(ulong steamId)
    {
        ...
        if (!string.IsNullOrEmpty(result.Knife))
			GPlayersKnife[steamId] = result.Knife;

        if (result.Glove != 0)
	        GPlayersGlove[steamId] = result.Glove;
    }
    ...
    private void ChangeWeaponAttributes(CBasePlayerWeapon weapon, CCSPlayerController player, bool isKnife = false)
	{
		...
		if (isKnife && !GPlayersKnife.ContainsKey(player.SteamID) || isKnife && GPlayersKnife[player.SteamID] == "weapon_knife") return;
		...
		if (!isKnife && weapon.CBodyComponent is { SceneNode: not null })
		{
	...
	private void GiveKnifeToPlayer(CCSPlayerController player)
	{
		...
		Plugin.AddTimer(1.0f, () =>
		{
			...
			var exist = GPlayersKnife.TryGetValue(player.SteamID, out var knife);
	...
	private void RefreshGloves(CCSPlayerController player)
	{
		...
		Plugin.AddTimer(0.06f, () =>
		{
			...
			{
				if (!player.IsValid()) return;
				if (!GPlayersGlove.TryGetValue(player.SteamID, out var gloveId) || gloveId == 0) return;
				...
				VFunc1.Invoke(item.NetworkedDynamicAttributes.Handle, "set item texture prefab", detail.Paint);
				VFunc1.Invoke(item.NetworkedDynamicAttributes.Handle, "set item texture seed", detail.Seed);
				VFunc1.Invoke(item.NetworkedDynamicAttributes.Handle, "set item texture wear", detail.Wear);

				item.Initialized = true;

				VFunc2.Invoke(pawn, "default_gloves", 1);
			}
			catch (Exception)
			{
				// ignored
	...
	{
		if (player.PlayerPawn.Value == null || !player.IsValid()) return null;
		
		return GetFixedArray<CHandle<CBaseViewModel>>(new CCSPlayer_ViewModelServices(player.PlayerPawn.Value.ViewModelServices!.Handle).Handle, "CCSPlayer_ViewModelServices", "m_hViewModel", 3);
	}
